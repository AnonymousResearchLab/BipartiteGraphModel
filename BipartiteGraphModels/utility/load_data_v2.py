...
class Data(object):
    ...
    def top_k(self, objects, weights, k=800):
    ...
    def weighted_sampling(self, objects, cs):
        idx = np.searchsorted(cs, np.random.rand())
        return objects[idx]
    ...
    def sample_v4(self):
        ...
        batch_neg_items = np.asarray([self.random_select(item, 1)[0] for item in self.u_i_neg_pool[batch_users]])

        batch_users_pos = np.asarray([self.weighted_sampling(user[0], user[1]) for user in self.u2u_adj[batch_users]])
        batch_users_neg = np.concatenate([self.random_select(user, 5) for user in self.u_u_neg_pool[batch_users]])

        batch_pos_items_pos = np.asarray([self.weighted_sampling(item[0], item[1]) for item in self.i2i_adj[batch_pos_items]])
        batch_pos_items_neg = np.concatenate([self.random_select(item, 5) for item in self.i_i_neg_pool[batch_pos_items]])

        batch_neg_items_pos = np.asarray([self.weighted_sampling(item[0], item[1]) for item in self.i2i_adj[batch_neg_items]])
        batch_neg_items_neg = np.concatenate([self.random_select(item, 5) for item in self.i_i_neg_pool[batch_neg_items]])

        return batch_users, batch_pos_items, batch_neg_items, batch_users_pos, batch_users_neg, batch_pos_items_pos, batch_pos_items_neg, batch_neg_items_pos, batch_neg_items_neg
...
class DataV2(object):
    def __init__(self, path, batch_size, num_neighbors=[10, 10]):
        self.path = path
        self.batch_size = batch_size
    ...
    def sample_path(self, center, path=['u','i'], neighbors_num=[25]):
        samples = [np.asarray(center)]
        current_center = center
        assert len(path) == len(neighbors_num) + 1, 'len(path) should be equals to len(neighbors_num) + 1'
        for i, neigh_num in enumerate(neighbors_num):
    ...
    def sample_v4(self):
        ...
        # batch_user_samples = self.sample_path(batch_users, path=['u', 'i'], neighbors_num=[30])
        # batch_pos_iu_samples = self.sample_path(batch_pos_items, path=['i', 'u'], neighbors_num=[30])
        # batch_neg_iu_samples = self.sample_path(batch_neg_items, path=['i', 'u'], neighbors_num=[30])
        # batch_pos_ii_samples = self.sample_path(batch_pos_items, path=['i', 'i'], neighbors_num=[30])
        # batch_neg_ii_samples = self.sample_path(batch_neg_items, path=['i', 'i'], neighbors_num=[30])

        # return batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples, batch_pos_ii_samples, batch_neg_ii_samples
        return batch_users, batch_pos_items, batch_neg_items

if __name__ == '__main__':
    data_loader = Data("Data/gowalla", 2)
    samples = data_loader.sample_v4()
