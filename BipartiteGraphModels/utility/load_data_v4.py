...
class DataV2(object):
    def __init__(self, path, batch_size, neighbors_num=[64]):
        self.path = path
        self.batch_size = batch_size
    ...
    def sample_source_target_nb(self, center, sample_num, source='u', target='i'):
        if source == 'u' and target == 'i':
            entities_mat = self.u2i_adj[center]
        elif source == 'u' and target == 'u':
            entities_mat = self.u2u_adj[center]
    ...
    # def sample_path_nb(self, center, path=['u','i']):
    #     samples = [center]
    #     current_center = center
    #     assert len(path) == len(self.neighbors_num) + 1, 'len(path) should be equals to len(neighbors_num) + 1'
    #     for i in range(len(self.neighbors_num)):
    #         source, target = path[i], path[i+1]

    #         if source == 'u' and target == 'i':
    #             entities_mat = self.u2i_adj[current_center]
    #         elif source == 'u' and target == 'u':
    #             entities_mat = self.u2u_adj[current_center]
    #         elif source == 'i' and target == 'i':
    #             entities_mat = self.i2i_adj[current_center]
    #         elif source == 'i' and target == 'u':
    #             entities_mat = self.i2u_adj[current_center]
    #         else:
    #             raise ValueError('source and target should be \'u\' or \'i\'')

    #         next_center = []
    #         for j in range(len(entities_mat)):
    #             idxs = np.random.randint(len(entities_mat[j]), size=self.neighbors_num[i])
    #             next_center.append(entities_mat[j][idxs])
    #         current_center = np.concatenate(next_center)
    ...
    def sample_v4(self):
        ...
        batch_user_samples_u = self.sample_path(batch_users, path=['u', 'u'])
        ...
        batch_pos_ii_samples = self.sample_path(batch_pos_items, path=['i', 'i'])
        batch_neg_ii_samples = self.sample_path(batch_neg_items, path=['i', 'i'])

        return batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples, batch_user_samples_u, batch_pos_ii_samples, batch_neg_ii_samples
    ...
    #         batch_user_samples = executor.submit(self.sample_path, batch_users, self.user_path)
    #         batch_user_samples_u = executor.submit(self.sample_path, batch_users, ['u','u'])
    #         batch_pos_iu_samples = executor.submit(self.sample_path, batch_pos_items, self.item_path)
    #         batch_neg_iu_samples = executor.submit(self.sample_path, batch_neg_items, self.item_path)
    #         batch_pos_ii_samples = executor.submit(self.sample_path, batch_pos_items, ['i', 'i'])
    #         batch_neg_ii_samples = executor.submit(self.sample_path, batch_neg_items, ['i', 'i'])

    #     return batch_user_samples.result(), batch_pos_iu_samples.result(), batch_neg_iu_samples.result(), batch_user_samples_u.result(), batch_pos_ii_samples.result(), batch_neg_ii_samples.result()
    ...
    #     batch_user_samples = self.sample_path_nb(batch_users, path=self.user_path, neighbors_num=self.neighbors_num)
    #     batch_user_samples_u = self.sample_path_nb(batch_users, path=['u', 'u'], neighbors_num=self.neighbors_num)
    #     batch_pos_iu_samples = self.sample_path_nb(batch_pos_items, path=self.item_path, neighbors_num=self.neighbors_num)
    #     batch_neg_iu_samples = self.sample_path_nb(batch_neg_items, path=self.item_path, neighbors_num=self.neighbors_num)
    #     batch_pos_ii_samples = self.sample_path_nb(batch_pos_items, path=['i', 'i'], neighbors_num=self.neighbors_num)
    #     batch_neg_ii_samples = self.sample_path_nb(batch_neg_items, path=['i', 'i'], neighbors_num=self.neighbors_num)

    #     return batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples, batch_user_samples_u, batch_pos_ii_samples, batch_neg_ii_samples
        # return batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples
...
if __name__ == '__main__':
    ...
    t1 = time()
    for i in range(100):
        batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples, batch_user_samples_u, batch_pos_ii_samples, batch_neg_ii_samples = data_loader.sample_v4()
    t2 = time()
    print('[NO Multi Thread]avg sample time cost:{}s'.format((t2 - t1)/100))

    t1 = time()
    for i in range(100):
        batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples, batch_user_samples_u, batch_pos_ii_samples, batch_neg_ii_samples = data_loader.sample_v5()
    t2 = time()
    print('[Multi Thread]avg sample time cost:{}s'.format((t2 - t1)/100))
    # batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples, batch_user_samples_u, batch_pos_ii_samples, batch_neg_ii_samples = data_loader.sample_v5()
    # print(batch_user_samples)
    # print(batch_pos_iu_samples)
    # print(batch_neg_iu_samples)
    # print(batch_user_samples_u)
    # print(batch_pos_ii_samples)
    # print(batch_neg_ii_samples)
