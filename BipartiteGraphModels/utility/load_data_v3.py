...
class DataV2(object):
    ...
    def construct_matrix(self):
        ...
        u2u2u_matrix = u2u_matrix * u2u_matrix
        ...
        self.u_soft_pos_u_adj = self.construct_skip_adj(u2u2u_matrix, self.u2u_adj, self.n_users)
        # self.u_soft_pos_u_adj = []
        # for u2u_list, u2u2u_list in zip(self.u2u_adj, self.u2u2u_adj):

        end_ts = time()
        print('Construct adjust matrix. Time: {:5.3f}'.format(end_ts - start_ts))

    def construct_skip_adj(self, e2e2e_matrix, e2e_adj, n_element):
        adj_list = []
        max_n = 0
        min_n = self.n_users + 1
        sum_n = 0
        for i in range(n_element):
            next_arr = np.asarray(
                e2e2e_matrix.indices[e2e2e_matrix.indptr[i]:e2e2e_matrix.indptr[i+1]])
            new_arr = np.setdiff1d(next_arr, e2e_adj[i])
            adj_list.append(new_arr)
            max_n = max(max_n, len(new_arr))
            min_n = min(min_n, len(new_arr))
            sum_n += len(new_arr)
        adj_list = np.asarray(adj_list)
        print('most:', max_n)
        print('least:', min_n)
        print('avg:', sum_n / self.n_users)
        return adj_list
    ...
    def sample_path(self, center, path=['u','i'], neighbors_num=[60]):
    ...
    def sample_v4(self):
        ...
        batch_user_samples = self.sample_path(batch_users, path=self.user_path, neighbors_num=self.neighbors_num)
        batch_pos_iu_samples = self.sample_path(batch_pos_items, path=self.item_path, neighbors_num=self.neighbors_num)
        batch_neg_iu_samples = self.sample_path(batch_neg_items, path=self.item_path, neighbors_num=self.neighbors_num)
...
if __name__ == '__main__':
    data_loader = DataV2("Data/gowalla", 2, neighbors_num=[2,1])
    batch_user_samples, batch_pos_iu_samples, batch_neg_iu_samples = data_loader.sample_v4()
